...
# 0) 项目路径（全局）
# ====================================================
BASE_DIR = Path(__file__).resolve().parent.parent  # 项目目录（保留）
PLAN_DIR = Path(__file__).resolve().parent

# 相对项目目录的路径，避免依赖机器上的绝对路径。
RAW_DATA_DIR = BASE_DIR / "raw_data"
ST_HISTORY_DIR = BASE_DIR / "ST_history"
ST_HISTORY_FILE = "st_stock_history_2020_2023.csv"

OUTPUT_ROOT_DIR = PLAN_DIR / "output"
...
RISK_KEYWORDS = [
    ...
    "重大违法", "立案", "调查", "监管", "处罚", "行政处罚", "纪律处分",
    "问询", "关注函", "监管函", "纪律", "自律监管",
    "亏损", "净利润为负", "扣非为负", "营收下降", "收入下降",
    "减值", "商誉减值", "资产减值", "信用减值",
    "违约", "逾期", "欠款", "无法偿还", "流动性",
    "诉讼", "仲裁", "执行", "强制执行", "冻结", "查封",
    "担保", "违规担保", "占用", "资金占用",
    "审计", "无法表示意见", "保留意见", "否定意见", "强调事项",
    "重大风险", "持续经营", "存在不确定性",
    "重整", "破产", "清算",
    "财务造假", "虚假记载", "重大遗漏",
]
...
# ====================================================
LLM_MODEL_PATH = BASE_DIR / "models" / "Qwen3-8B"
...
FUSION_FILTER_HIGH_TH = 5
FUSION_FILTER_MEDIUM_TH = 3
FUSION_DECAY_PER_DAY = 0.3
...
# ====================================================
BLACKLIST_ENTRY_THRESHOLD = 80        # 准入阈值：新分 >= 80 才入黑名单
BLACKLIST_RETAIN_THRESHOLD = 65       # 准出阈值：记忆分 < 60 才清出黑名单
