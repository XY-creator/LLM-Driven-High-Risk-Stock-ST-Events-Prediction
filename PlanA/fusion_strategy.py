...
def apply_decay_memory_scores(
    ...
    baseline 风格记忆衰减:
    - 每日先衰减
    - 当天有新分时，用 max(旧分, 新分) 更新
    - 未入记忆前，只有新分 >= threshold 才入记忆
    - 低于 threshold 的记忆直接清空为 0
    """
    decay = float(decay_per_day)
    entry_thr = float(entry_threshold)
    retain_thr = float(retain_threshold) if retain_threshold is not None else entry_thr

    if not isinstance(raw_score_s, pd.Series):
        raise ValueError("raw_score_s 必须是 pd.Series。")

    out = raw_score_s.copy()
    out.loc[:] = np.nan

    output_mask = np.asarray(output_mask, dtype=bool)
    out_idx = raw_score_s.index[output_mask]
    if len(out_idx) == 0:
        return out.fillna(0.0).astype(np.float32)

    out_raw = raw_score_s.loc[out_idx].sort_index(level=["stock_code", "date"])

    def _fuse_one_stock(s):
        state = 0.0
        ...
        return pd.Series(vals, index=s.index, dtype=np.float32)
    ...
    out.loc[out_idx] = out_fused.reindex(out_idx).astype(np.float32)
    out.loc[~output_mask] = 0.0
